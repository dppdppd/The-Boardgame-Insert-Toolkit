...
# BIT GUI (Experimental)

BIT GUI is a desktop application for visually editing BIT `.scad` files. It provides a structured, form-based editor where the `.scad` file remains the single source of truth.

### Features
- **Line-based editing**: every line of the SCAD file is represented individually — either as a native control or as editable raw text
- **Schema-driven controls**: recognized BIT keys (`BOX_SIZE_XYZ`, `LBL_TEXT`, `CMP_SHAPE`, etc.) render as appropriate form controls (text fields, checkboxes, dropdowns, XYZ inputs)
- **Structural bracket tracking**: `[` and `]` brackets are parsed and labeled with their hierarchy role (element, params, label, lid, component)
- **Collapsible sections**: elements, labels, lids, and component blocks can be collapsed
- **Virtual defaults**: all schema keys for each context are shown as dimmed virtual rows; changing a virtual value materializes it as a real line
...
**Windows note**: if Electron crashes with `GPU process isn't usable`, the start script already includes `--disable-gpu --no-sandbox` flags. If issues persist, set the environment variable `BITGUI_OPEN=path/to/file.scad` before running.

### Architecture

BIT GUI is an Electron + Svelte 5 app. The project model is an array of `Line` objects — one per non-blank line of the SCAD file. Each line is independently classified:

| Kind | Meaning | UI |
|------|---------|-----|
| `kv` | Recognized `[ KEY, VALUE ]` | Native control (checkbox, number, text, dropdown, etc.) |
| `global` | Known global assignment | Inline control |
| `open` / `close` | Structural bracket | Labeled, collapsible, undeletable |
| `include` / `marker` / `makeall` | Regenerated by generator | Muted badge |
| `raw` | Unrecognized | Editable text (adjacent raw lines merge into one textarea) |

Key files:
- `bit-gui/importer.js` — parses SCAD into Line objects with bracket-depth tracking
- `bit-gui/src/lib/scad.ts` — generates SCAD output from Line array
- `bit-gui/src/lib/stores/project.ts` — Svelte store with Line CRUD operations
- `bit-gui/src/App.svelte` — main UI rendering all line types
- `bit-gui/schema/bit.schema.json` — v4 key definitions driving controls and virtual defaults
- `bit-gui/main.js` — Electron main process (menu, file I/O, recent files)
