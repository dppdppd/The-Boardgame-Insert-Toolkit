...
{
    echo( str( "BIT: physical validation: ", context, " ", message ) );
}
...
function __components_overlap_xy( a, b, box_size, wall_thickness ) =
    __ranges_overlap(
        __component_validation_min( a, box_size, wall_thickness, k_x ),
        __component_validation_max( a, box_size, wall_thickness, k_x ),
        __component_validation_min( b, box_size, wall_thickness, k_x ),
        __component_validation_max( b, box_size, wall_thickness, k_x )
    ) &&
    __ranges_overlap(
        __component_validation_min( a, box_size, wall_thickness, k_y ),
        __component_validation_max( a, box_size, wall_thickness, k_y ),
        __component_validation_min( b, box_size, wall_thickness, k_y ),
        __component_validation_max( b, box_size, wall_thickness, k_y )
    );
...
module __ValidateLidPhysical( lid, ctx, wall_thickness )
{
...
module __ValidateComponentPhysical( comp, ctx, box_ctx, box_size, wall_thickness )
{
    if ( __component_physical_values_ok( comp ) )
    {
        ...
        if ( __component_physical_position_ok( comp ) && __component_physical_supported_footprint( comp ) )
        {
            _min_x = __component_validation_min( comp, box_size, wall_thickness, k_x );
            _max_x = __component_validation_max( comp, box_size, wall_thickness, k_x );
            _min_y = __component_validation_min( comp, box_size, wall_thickness, k_y );
            _max_y = __component_validation_max( comp, box_size, wall_thickness, k_y );
...
module __ValidateElement( element, element_name, element_type = undef )
{
    ...
    if ( element_type == DIVIDERS )
    {
        ...
        if ( __value( element, ENABLED_B, default = true ) )
    ...
    else
    {
        ...
        if ( is_list( lid ) && len( lid ) > 0 )
        {
            ...
            if ( is_num( _wt ) )
                __ValidateLidPhysical( lid, _lid_ctx, _wt );
        }
        ...
        for ( i = [ 0 : max( len( element ) - 1, 0 ) ] )
        {
            if ( is_list( element[i] ) && len( element[i] ) >= 2 )
            {
                if ( element[i][ k_key ] == BOX_FEATURE )
                {
                    ...
                    if ( __value( comp, ENABLED_B, default = true ) &&
                         __all_num_list( _box_size, 3 ) && is_num( _wt ) )
                        __ValidateComponentPhysical( comp, _comp_ctx, _ctx, _box_size, _wt );
                }
            }
        }

        if ( __all_num_list( _box_size, 3 ) && is_num( _wt ) )
        ...
        module MakeLid() 
        {
            ...
            lid_sliding_closed_position = !m_lid_slides_x ?
                ( m_lid_slide_side == FRONT ?
                    [ m_sliding_lid_rail_side_clearance, 0, m_box_size[ k_z ] + m_sliding_lid_fit_tolerance ] :
                    [ m_sliding_lid_rail_side_clearance, m_sliding_lid_stop_clearance, m_box_size[ k_z ] + m_sliding_lid_fit_tolerance ] ) :
                ( m_lid_slide_side == LEFT ?
                    [ 0, m_sliding_lid_rail_side_clearance, m_box_size[ k_z ] + m_sliding_lid_fit_tolerance ] :
                    [ m_sliding_lid_stop_clearance, m_sliding_lid_rail_side_clearance, m_box_size[ k_z ] + m_sliding_lid_fit_tolerance ] );
