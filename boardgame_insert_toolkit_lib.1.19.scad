...
g_lid_thickness = g_wall_thickness; // default = g_wall_thickness
...
function __label_size_raw( label ) = __value( label , "size", default = "auto" );
...
function __label_rotation( label ) = __label_rotation_raw( label ) + 
   ( __label_placement_is_left( label ) ? -90 : __label_placement_is_right( label ) ? 90 : 0 );
...
function __label_placement_is_above( label ) = __label_placement_raw( label ) == "above";
function __label_placement_is_below( label ) = __label_placement_raw( label ) == "below";
...
module MakeBox( box )
{
    ...
    module MakeLayer( component, layer = "" )
    {
        ...
        function __c_m_s( side ) = __value( component, "margin", default = [1,1,1,1] )[ side ];
        function __component_has_margin( D ) = D == X ?
                [ __c_m_s( LEFT ), __c_m_s( RIGHT ) ] :
                [ __c_m_s( BOTTOM ), __c_m_s( TOP )];
        ...
        function __component_cutout_side( side ) = __value( component, "cutout_sides", default = [0,0,0,0] )[ side ];
        ...
        function __component_has_cutout( D ) = __value( component, "finger_cutout", default = [false, false] )[ D ];
...
// end delete me
        ...
        function __component_size( D )= ( D == Z ) ? __compartment_size( Z ) : ( __compartment_size( D )* __compartments_num( D )) + ( __partitions_num( D )* __component_padding( D ));
        ...
        m_lid_thickness = ( m_box_has_thin_lid ? 0.6 : g_lid_thickness ) - m_tolerance;
...
/////////////////////////////////////////
        ...
        module InnerLayer()
        {
            ...
            else if ( m_is_final_component_subtractions )
            {
                ...
                InEachCompartment( )
                {
                    if ( __component_cutout_side( BOTTOM ))
                        MakeFingerCutout( BOTTOM );

                    if ( __component_cutout_side( TOP ))
                        MakeFingerCutout( TOP );
...
///////////////////////
        ...
        module MakeLid() 
        {
            ...
            module MakeLidText( offset = 0, thickness = m_lid_thickness )
            {
                ...
                theta = __label_rotation( m_box_label );

                new_width = ( cos( theta ) * ( __lid_external_size( X )/1.5 )) + ( sin( theta ) * ( __lid_external_size( Y )/1.5 )); 
                width = __label_size_is_auto( m_box_label ) ? min( 100, new_width ) + offset : 0;
            ...
            translate( g_b_visualization ? lid_vis_position : lid_print_position ) 
                RotateAboutPoint( g_b_visualization ? 180 : 0, [0, 1, 0], [__lid_external_size( X )/2, __lid_external_size( Y )/2, 0] )
                {
                    ...
                    text_offset = __label_size( m_box_label) ;
    ...
    module MakeRoundedCube( vec3, radius){
        ...
        module MakeFingerCutout( side )
        {
            ...
            {
                cutout_y = __component_padding(Y) + min(10, __compartment_size(Y));
                cutout_x = max( 20, __compartment_size( X )/3 );

                pos_x = __compartment_size( X )/2  - cutout_x/2;
                pos_y = side == BOTTOM ? - __component_padding( Y ) : - cutout_y;
                ...
                {
                    pos_y2 =  __compartment_size( Y ) + __component_padding( Y ) + pos_y;
            ...
            {
                cutout_x = __component_padding(X) + min(10, __compartment_size(X));
                cutout_y = max( 20, __compartment_size( Y )/3 );

                pos_x = side == LEFT ? - __component_padding( X ) : -cutout_x;
                ...
                {
                    pos_x2 =  __compartment_size( X ) + __component_padding( X ) + pos_x;
       ...
       module AddFillets()
        ...
        {
            label_is_vertical = __label_placement_is_left( m_component_label ) || __label_placement_is_right( m_component_label );
            label_is_horizontal = __label_placement_is_above( m_component_label ) || __label_placement_is_below( m_component_label );            
            label_is_center = __label_placement_is_center( m_component_label );
 
            theta = __label_rotation( m_component_label );

            factor = 1.2 + (1.5 * abs(tan( theta % 90 )) );

            center_calculated_width = ( abs(cos( theta )) * ( __compartment_size( X )/factor )) + ( abs(sin( theta )) * ( __compartment_size( Y )/factor )); 

            // max_height = label_is_vertical ? __component_padding( X ) : 
            //             label_is_horizontal ? __component_padding( Y ) : 0;

            max_width = label_is_vertical ? __component_padding( Y ) : 
                        label_is_horizontal ? __component_padding( X ) : 0;

            width = __label_size_is_auto( m_component_label ) ? label_is_center ? center_calculated_width : max_width : 0;

            RotateAboutPoint( theta, [0,0,1], [0,0,0] )
        ...
        module MakeLabel( x = 0, y = 0 )
        {
            ...
            else if ( __label_placement_is_below( m_component_label) )
            {                
                translate( [ __compartment_size(X)/2, -__component_padding( Y )/4, __partition_height( Y ) + z_pos] )
                    children();
            }
            else if ( __label_placement_is_above( m_component_label) )
            {
