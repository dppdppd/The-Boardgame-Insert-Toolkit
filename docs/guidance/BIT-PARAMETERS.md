...
`G_PRINT_LID_B`, `G_PRINT_BOX_B`, `G_ISOLATED_PRINT_BOX`, `G_VISUALIZATION_B`, `G_VALIDATE_KEYS_B`, `G_WALL_THICKNESS` (default 2.0mm), `G_LID_THICKNESS` (default follows wall thickness), `G_TOLERANCE`, `G_TOLERANCE_DETENT_POS`, `G_DEFAULT_FONT`, `G_PRINT_MMU_LAYER`

File-scope `$g_*` variables provide defaults; data array entries override them via `Make()`. `G_VALIDATE_KEYS_B` enables key/type validation plus physical validation warnings for known fit and printability risks, including sliding-lid detent sizing/placement and fragile side, rounded finger, bottom, and corner cutout dimensions.
