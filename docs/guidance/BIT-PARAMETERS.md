...
File-scope `$g_*` variables provide defaults; data array entries override them via `Make()`. `G_VALIDATE_KEYS_B` enables key/type validation plus physical validation warnings for known fit and printability risks.
